...
function getContentAggregationQuery(context, roles, options = {}) {
  ...
  const isArchivedQueryStage = [{ // only get not archived
    $match: {
      $or: [
        { 'newContent.block.properties.archived': { $exists: false } },
        {
          $and: [
            { 'newContent.block.properties.archived': { $exists: true } },
            { 'newContent.block.properties.archived': false },
          ]
        }
      ]
    }
  ...
  const isActiveQueryStage = [{ // only get active // the active-property hides or shows blocks on the page
    $match: {
      $or: [
        { 'newContent.block.properties.active': { $exists: false } },
        {
          $and: [
            { 'newContent.block.properties.active': { $exists: true } },
            { 'newContent.block.properties.active': true },
          ]
        }
      ]
    }
  ...
  const stages = [
    ...
    // { $set: { 'root.computed.contentAsMarkdown': { $concat: "$contentAsMarkdown" } } },
    { $replaceRoot: { newRoot: '$root' } },
  ]
